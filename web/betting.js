...
(function () {
  ...
  function labView() {
    ...
    var intro =
      ...
      '<p><button id="bet-recal" class="btn primary btn-sm">🔧 Recalibrate model</button> ' +
      '<span class="muted">searches the parameters against history and applies the best — ~10s NFL, ~1 min college.</span>' +
      (r.calibrated_at ? ' <span class="muted">Last calibrated ' + esc(r.calibrated_at.replace("T", " ")) + ".</span>" : "") +
      "</p></div>";
  ...
  // ---- strategy tracker ---------------------------------------------
  var STRATS = ["spread", "moneyline", "value"];
  ...
  function stratView() {
    ...
    var intro =
      ...
      '<p>Three flat-<strong>$' + r.stake + '</strong> strategies, graded every week. Each game’s bets are <strong>locked ' + r.lock_hours + 'h before kickoff</strong> using the model and line as they stand then — never revised after. ' +
      '<button id="bet-locknow" class="btn btn-sm" title="Lock bets for any game kicking off within ' + r.lock_hours + 'h now">🔒 Lock due bets now</button></p>' +
      "</div>";

    if (!r.n_locks) {
      return intro + '<div class="bet-acc-empty muted">No bets locked yet. Bets lock automatically ~' + r.lock_hours + 'h before each game (the app does this in the background while it’s open, or click “Lock due bets now”). Come back once this week’s games are inside that window.</div>';
    }
    ...
    var head = '<tr><th>Week</th>' + STRATS.map(function (s) {
      return "<th>" + cap(s === "value" ? "Value" : s) + "</th>";
    }).join("") + "</tr>";
    var rows = r.weeks.map(function (w) {
      ...
      var detail = open ? '<tr class="bet-wk-detail"><td colspan="4">' + weekDetail(w) + "</td></tr>" : "";
  ...
  function shortStrat(s) { return s === "value" ? "High conviction" : (s === "moneyline" ? "Moneyline" : "Spread"); }
  ...
  function weekDetail(w) {
    return STRATS.map(function (s) {
      ...
      return (
        ...
        '<div class="bet-detail-head">' + esc(state.strategies.strategy_labels[s]) +
        ' <span class="muted">' + esc(t.record || "0-0") + " · </span>" +
        '<span class="bet-ev ' + pnlCls(t.profit) + '">' + money2(t.profit) + "</span></div>" +
        '<table class="bet-table bet-detail-table"><thead><tr><th>Game</th><th>Pick</th><th>Price</th><th>Result</th><th>P&amp;L</th></tr></thead><tbody>' +
